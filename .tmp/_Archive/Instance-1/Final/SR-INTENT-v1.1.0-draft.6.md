...
solver_ralph:
  ...
  supersedes: ["SR-INTENT@1.1.0-draft.4"]
  created: "2026-01-09"
  updated: "2026-01-11"
...
# SOLVER-Ralph Design Intent (Why²) — v1.1.0-draft.6
...
### 1.1.0-draft.6 (2026-01-11)
- Added explicit SR-PLAN vs SR-DIRECTIVE disambiguation to the determinism model.

### v1.1.0-draft.4 (2026-01-11)

- Clarified that IPS and SAPS are not merely narrative artifacts: under SR-PARADIGM S0 they are committed as `record.problem_statement` (stage=`IPS`/`SAPS`) and referenced downstream via typed `refs[]`, preventing "ghost meaning".
- Tightened the Trust Model and Problem Statement Compilation rationale to treat paraphrases as non-authoritative unless they point to the committed record.
...
**Clarification (anti-ghost-meaning):** In the SOLVER-Ralph paradigm, IPS and SAPS are *authoritative* only when they exist as committed, typed records. SR-PARADIGM requires S0 outputs to be stored as `record.problem_statement` with the appropriate `stage` value and to be referenced downstream via typed `refs[]`. Re-paraphrasing or summarizing the problem statement is allowed for readability, but it must not replace the committed record as the semantic foundation for binding work.
