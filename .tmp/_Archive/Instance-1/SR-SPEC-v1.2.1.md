...
solver_ralph:
  ...
  - SR-SPEC@1.0.0
---
# SOLVER-Ralph Technical Specification v1.2.0
...
**Satisfies Contracts:** C-ARCH-1..C-ARCH-3; C-TB-1..C-TB-6; C-LOOP-1..C-LOOP-4; C-CTX-1..C-CTX-2; C-EVT-1..C-EVT-7; C-EVID-1..C-EVID-5; C-OR-1..C-OR-7; C-VER-1..C-VER-4; C-EXC-1..C-EXC-5; C-SHIP-1; C-DEC-1; C-META-1..C-META-3.
...
```jsonc
{
  ...
  "stream_seq": 42,
  "global_seq": 184467,              // optional but RECOMMENDED
  ...
  "refs": [                           // typed references (dependency + audit)
    ...
    {"kind": "EvidenceBundle", "id": "sha256:...", "rel": "produced", "meta": {"content_hash": "sha256:..."}}
  ],
...
```jsonc
{
  "kind": "GovernedArtifact|Candidate|OracleSuite|EvidenceBundle|Record|Decision|Deviation|Deferral|Waiver|Loop|Iteration|Run|Freeze",
  "id": "string",
  "rel": "about|depends_on|supported_by|produces|verifies|approves|acknowledges|supersedes|in_scope_of|affects|stale|root_cause|relates_to",
  "meta": { "key": "value" }
}
```
...
```jsonc
{
  ...
  "dependency_refs": {
    "governed_artifacts": [
      {"id": "SR-CONTRACT", "version": "1.0.0-draft.5", "content_hash": "sha256:..."}
    ],
    "active_exceptions": [
...
```jsonc
{
  ...
  "supersedes": ["SR-CONTRACT@1.0.0-draft.4"],
  "is_current": true
}
```
...
```jsonc
{
  ...
  "artifact_manifest": [
    { "artifact_id": "SR-CONTRACT", "version": "1.0.0-draft.5", "content_hash": "sha256:<hex>" },
...
```jsonc
{
  ...
  "root": { "kind": "GovernedArtifact", "id": "SR-CONTRACT@1.0.0-draft.5" },
  "reason_code": "GOVERNED_ARTIFACT_CHANGED",
  "reason_detail": "SR-CONTRACT version changed from 1.0.0-draft.4 to 1.0.0-draft.5",
  "dependents": [
...
```jsonc
{
  ...
  "reason": "Candidate depends on updated governed artifact SR-CONTRACT@1.0.0-draft.5"
}
```
...
- `POST /approvals` → Create a Portal approval record (human only)  
  ...
  Notes: For ReleaseApprovalPortal closeout, `evidence_refs[]` SHOULD include the validation Evidence Bundle(s) relied upon, and `attestations.validation_assessed` SHOULD be set to `true`.
  Where present, `attestations` MAY include: `{ validation_assessed?: boolean }`.
...
### Appendix C: Evidence integrity conditions (minimum set)
