...
solver_ralph:
  ...
  - artifacts
---
# SOLVER-Ralph Types v3.3.0-draft.5
...
- Clarified the Problem Statement typing rule to align with SR-PARADIGM: IPS and SAPS are commitment objects stored as `record.problem_statement` and treated as authoritative inputs for governance authoring.
...
- Clarified that `record.problem_statement` is not boot-required for the governance substrate, but is required to proceed beyond S0 (S1+) under SR-PARADIGM; added an explicit `SR-PARADIGM S1+ required?` flag in §4.3.
...
**Typing rule:**
- Under SR-PARADIGM S0, IPS and SAPS MUST be captured as versioned `record.problem_statement` artifacts with `ext.problem_statement.stage` set appropriately (`IPS` / `SAPS`).
...
#### 1.3.4 Paradigm vs Instance

- **Paradigm**: The governed set of artifacts (and their controlled evolution) that defines *what counts* as valid work, evidence, authority, and state transitions.
  - The paradigm is expressed as governed documents (e.g., SR-TYPES, SR-INTENT, SR-CONTRACT, SR-SPEC, SR-DIRECTIVE, SR-CHANGE, SR-README, and any additional canonical artifacts selected by SR-README and governed by SR-CHANGE).
  - The paradigm defines **deterministic validity rules**; it does not “execute itself.”

- **Instance**: A concrete software implementation that enforces the paradigm’s rules and records commitments (events, evidence, approvals, freezes, and other records).
  - An instance is replaceable (within constraints) if it preserves the paradigm’s semantics and auditability properties.
  - Actor choices (human/agent/system) are not assumed deterministic; the instance’s enforcement of validity **must be deterministic** given recorded inputs.

**Constraint:** Do not use these terms to smuggle in implementation specifics into paradigm-level governance. Paradigm-level artifacts may define required capabilities (e.g., replayability, evidence linkage), while implementation mechanics belong in SR-SPEC and adapters.
## 2. Authority Model
...
**SR-PARADIGM S1+ required?** indicates whether the artifact must exist to *proceed beyond S0* (i.e., begin governance authoring/instantiation stages) under SR-PARADIGM.

| Type Key | Boot-required? | SR-PARADIGM S1+ required? | Authority Kind | Normative Status | Examples |
|---|---:|---:|---|---|---|
...
solver_ralph:
  ...
  updated: "{YYYY-MM-DD}"
  refs:
    # For a plan template, refs SHOULD include SR-TYPES and SR-PARADIGM.
