...
solver_ralph:
  ...
  title: "SOLVER-Ralph"
  version: "1.0.8-draft.1"
  supersedes: ["SR-README@1.0.7-draft.1"]
  status: "draft"
...
- **Architectural Contract (SR-CONTRACT)** — `SR-CONTRACTS-v1.1.0-draft.1.md`
...
### Canonical governed set (registered in SR-README; change-controlled by SR-CHANGE)

> **Registry note:** SR-README is the **canonical index** of what this SOLVER‑Ralph instance treats as “canonical governed artifacts.”
> SR-TYPES registers **type keys** and metadata conventions; SR-CHANGE governs **how** the set may be changed.
> If any governed artifacts conflict, **do not interpret**—route a governed change via SR-CHANGE.

**Conflict precedence (binding technical semantics):** SR-CONTRACT > SR-SPEC > SR-DIRECTIVE > SR-INTENT > SR-README

- **SR-TYPES** — document type system and templates  
  `SR-TYPES-v3.3.0-draft.8.md`

- **SR-INTENT** — directional “why” and interpretive guidance  
  `SR-INTENT-v1.1.0-draft.6.md`

- **SR-ETT** — epistemic trust topology (nine membranes; trust as emergent property)  
  `SR-ETT_v1.0.1.md`

- **SR-AGENTS** — what an “agent” is in this paradigm (non-authoritative, proposal-first)  
  `SR-AGENTS.md`

- **SR-PARADIGM** — problem-statement compilation + governance authoring guidance (RPS→IPS→SAPS→INPS)  
  `SR-PARADIGM-v1.0.0-draft.10.md`

- **SR-CONTRACT** — binding architecture invariants  
  `SR-CONTRACTS-v1.1.0-draft.1.md`

- **SR-SPEC** — binding technical mechanics (events/APIs/state machines)  
  `SR-SPEC-v1.2.1.md`

- **SR-PLAN** — binding decomposition of the SAPS into packages + deliverables (what must be built)
  `SR-PLAN-v1.0.0-draft.2.md`

- **SR-DIRECTIVE** — binding operational protocol for implementers  
  `SR-DIRECTIVE-v1.1.1.md`
...
## SR-PLAN and SR-DIRECTIVE

- **SR-PLAN** answers **what must be built**: a typed decomposition of the authoritative SAPS into packages and deliverables (each deliverable intended to be loop-completable, with dependencies and evidence expectations).
- **SR-DIRECTIVE** answers **how work is run**: the execution discipline (linear/concurrent sequencing), gates/triggers/budgets, verification profiles, and completeness rules that progress the system toward resolution.

Execution strategy may adapt, but required deliverables must not change without an explicit plan change routed through SR-CHANGE.
...
2) Establish the **alignment substrate** (canonical governed artifacts):
   - **SR-TYPES** (ontology)
   - **SR-INTENT** (purpose / rationale anchor)
   - **SR-ETT** (trust topology)
   - **SR-AGENTS** (agent object model + non-authoritativeness)
   - **SR-PARADIGM** (problem-statement compilation + authoring discipline)
   - **SR-CONTRACT** (binding invariants)
   - **SR-SPEC** (mechanics)
   - **SR-DIRECTIVE** (execution protocol)
   - **SR-CHANGE** (evolution control)
   - **SR-README** (registry + current selections)
3) Choose an oracle suite and define your first verification profile(s).
4) Implement minimal adapters:
   - artifact store (filesystem is fine),
   - event store (append-only),
   - oracle runner (subprocess/container),
   - agent worker runtime (sandboxed).
