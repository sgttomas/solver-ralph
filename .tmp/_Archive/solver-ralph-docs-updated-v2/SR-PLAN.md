...
# SR-PLAN — Build Decomposition

## 0. Change Log

- 2026-01-11: Imported from split planning notes; normalized ontology, removed obsolete instance/template framing, and aligned precedence + exception handling.

## 1. Planning inputs (non-binding)

This document is a **build decomposition**. It does not define platform meaning, invariants, or runtime mechanics.

It is compiled against the platform-definition documents:
...
And it should be executed under the workflow documents:
...
If this plan becomes ambiguous or contradictory, **escalate to Human Authority** and (if needed) record an entry in SR-EXCEPTIONS.
...
- **Depends on:** *(none)*
- **Refs required:** `PS-SOLVER-RALPH-BUILD-SAPS`, `SR-CONTRACT`, `SR-SPEC`, `SR-DIRECTIVE`
