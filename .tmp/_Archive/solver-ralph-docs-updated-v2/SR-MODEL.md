...
The workflow documents (SR-PLAN, SR-DIRECTIVE, SR-AGENTS, SR-CHANGE, SR-EXCEPTIONS) are **governed build scaffolding**. They instruct agents and control execution during the build, but they are **not part of the delivered platform runtime semantics**.

When instructions are ambiguous, agents must route by decision type (platform-definition vs build-execution) and escalate to **Human Authority** when needed.
