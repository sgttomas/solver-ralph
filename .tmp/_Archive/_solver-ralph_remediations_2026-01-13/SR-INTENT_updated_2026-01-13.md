...
**Terminology note:** This document uses canonical terms per **SR-CONTRACT §2.11** and type keys per **SR-TYPES**. If wording here conflicts with CONTRACT/TYPES/SPEC, those governed artifacts control.
...
**Realization dependencies (where this is enforced):**
- **SR-SPEC**: event types and binding derivations (EvidenceBundleRecorded / ApprovalRecorded / FreezeRecordCreated).
- **SR-EVENT-MANAGER**: projections that compute trust states from the event stream + governed policy snapshots.
- **SR-DIRECTIVE**: gate registry + routing when verification is missing, stale, or blocked.
- **SR-CONTRACT**: invariants for what “Verified / Approved / Shippable” may mean.
...
**Non-normative guidance:** This section is operator guidance for agent/worker behavior. It does not create new binding semantics beyond SR-CONTRACT/SR-SPEC/SR-DIRECTIVE.
