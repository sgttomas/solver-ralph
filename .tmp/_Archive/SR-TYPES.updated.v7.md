...
**Scope note:** This document defines both (a) runtime-intrinsic *Platform Domain Types* (used by the running platform) and (b) build/governance typing used to manage the spec set. When in doubt: the runtime schema realizations live in **SR-SPEC**, and the invariants they must uphold live in **SR-CONTRACT**.
...
| **Loop Record** | semantic-ralph-loop iteration summary | SR-SPEC |
...
**Commitment Object note:** “Commitment Object” is a durability/referenceability category: a content-addressed, durable platform artifact. It spans both `normative_status: record` and `normative_status: evidence` artifacts (e.g., an Evidence Bundle is a Commitment Object, but remains evidence in normative status).
...
- **Portal** is the canonical term for a human decision trust boundary (aka **Authority Port** / **Trust Boundary** in older phrasing). Use *Portal* in type keys and prose; treat *Authority Port* as an alias.
- The canonical surface form for a Semantic Ralph Loop / work unit is `semantic-semantic-ralph-loop` (other variants are aliases).
- Canonical evidence artifact name is **`domain.evidence_bundle`** (avoid introducing parallel names).
...
- `governance.charter` — charter-level governing document. `authority_kind: record`; `normative_status: record`.


**Axis note:** `authority_kind` and `normative_status` are orthogonal.

- `authority_kind` describes *what kind of authority* an artifact could carry (content/process/record/config/index).
- `normative_status` describes *how the artifact is used normatively* in governance and runtime (e.g., record vs evidence vs directional).
...
| `governance.work_surface_spec` | content | normative | Work Surface definitions and binding rules (SR-WORK-SURFACE) |
| `governance.procedure_kit` | process | normative | Procedure template kit (stages, required outputs, required suites) (SR-PROCEDURE-KIT) |
| `governance.event_manager_spec` | content | normative | Event Manager spec: deterministic projections of status/eligibility from events + governed inputs |
| `governance.agent_worker_contract` | process | normative | Agent/worker runtime contract: invocation inputs, obligations, stop-trigger handling, and event emission requirements |
| `governance.semantic_oracle_spec` | content | normative | Semantic oracle spec: suite identity, semantic-set binding, required outputs, and determinism requirements |
...
| `domain.loop_record` | record | record | semantic-ralph-loop iteration summary |
...
solver_ralph:
  ...
  ext:
    usage_guide:
      purpose: ""
      core_abstraction: "semantic-ralph-loop"
---
...
Oracle verification output. Canonical artifact: `domain.evidence_bundle`.
...
│  - domain.loop_record — semantic-ralph-loop iteration summary             │
