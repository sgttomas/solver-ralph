...
solver_ralph:
  ...
  ext:
    arch_contract:
      ...
      scope:
        ...
        archetype: true
      notes:
      - 'Draft.5 incorporates select clarifications from draft.3 where they improve
        enforceability and cross-doc coherence: event-store source-of-truth, stable
        actor identity + approval record minimum fields, evidence bundle minimum manifest
        aligned to evidence.gate_packet, and waiver lifecycle/expiry expectations.'
      - "Draft.5 adds REPEATED_FAILURE (N\u22653, Directive-defined) as a mandatory\
        \ stop-the-line trigger, aligning the loop governor with conservative escalation\
        \ principles."
      - Draft.5 clarifies that ORACLE_TAMPER and ORACLE_ENV_MISMATCH cannot be bypassed
        by waivers; resolution requires restart or explicit suite rebase.
      - Draft.6 makes iteration context provenance binding via `IterationStarted.refs[]`
        (SYSTEM-only) and prohibits ghost inputs as a contract requirement.
      - v1.0.1-draft.1 adds SR-TYPES-aligned `ext.arch_contract.*` index fields, adds
        a human-readable Invariants Index, and upgrades the Conformance Checklist
        into a requirement-by-requirement table (no new contract requirements).
      - Frontmatter ext.arch_contract.trust_boundaries enumerates contract-mandated
        MINIMUM required Portals (not an exhaustive list).
      invariants:
      - C-ARCH-1
      - C-ARCH-2
      - C-ARCH-3
  ...
  - kind: GovernedArtifact
    ...
    rel: supported_by
    meta:
      type_key: governance.design_intent
  ...
  - kind: GovernedArtifact
    ...
    meta:
      ...
      version: 1.0.7-draft.1
---
# SOLVER-Ralph Architectural Contract v1.1.0-draft.1
...
**Interpretation Guidance:** Consult **SR-INTENT** for rationale and tradeoffs. Consult **SR-TYPES** for artifact taxonomy, metadata conventions, and the authority model. Consult **SR-PARADIGM** for governance authoring workflow context (non-authoritative here).
---
...
- **Normative tightening:** clarifies **Gate vs Portal** terminology and anchors **Approval** explicitly to a **Portal** (trust boundary).
- **Ontology completion (binding definitions):** adds definitions for **Proposal vs Commitment Object** and the 4-way distinction **Verification / Evaluation / Validation / Assessment**.
- **Epistemic membrane hardening:** adds **C-TB-7** to state that Evaluation/Assessment are **non-binding** and MUST NOT be treated as substitutes for Portal Approval/Decision.
- **Evidence availability invariant:** adds **C-EVID-6** and a new integrity condition **EVIDENCE_MISSING** (non-waivable) to prevent Verified/Approved/Shippable claims from resting on non-retrievable evidence.
- **Waiver guardrail:** clarifies that Gate Waivers apply only to explicit **FAIL** outcomes and MUST NOT bypass integrity conditions (including **EVIDENCE_MISSING**).
- Updates Invariants Index, frontmatter `ext.arch_contract.*` lists, and the Conformance Checklist Table accordingly.

### 1.0.1-draft.1 (2026-01-11)

- Adds SR-TYPES-aligned **frontmatter index fields**: `ext.arch_contract.invariants`, `ext.arch_contract.trust_boundaries`, `ext.arch_contract.non_waivable_conditions`.
- Adds a human-readable **Invariants Index** (navigation-only; no new requirements).
- Replaces the Conformance Checklist bullets with a **requirement-by-requirement Conformance Checklist Table** (no new requirements).
...
This section is **navigation-only**. It introduces no new binding meaning; it points to the existing requirements.
...
- **Approved:** a human actor authorizes a Candidate (or governed artifact change) at a **Portal** (trust boundary) with reference to evidence (and any waivers/exceptions).
...
- **Gate Waiver:** a binding record that permits progression despite failure of a required oracle/gate, under explicit human approval and explicit scope. Waivers apply only to explicit **FAIL** outcomes and MUST NOT be used to bypass integrity conditions (see §2.5).
...
- **Proposal:** any draft, statement, or work product whose meaning is not yet stabilized as a durable, attributable object in the system. Proposals may inform decisions but are **not** a valid substrate for binding state.
- **Commitment Object:** a durable, content-addressed, referenceable object recorded in the system and safe for downstream reliance (e.g., governed artifacts, Candidates, Evidence Bundles, Approvals, Freeze Records, Decision Records, and Exception Records).

**Rule of thumb:** binding claims (Verified/Approved/Shippable) MUST be derivable from commitment objects referenced in the event/graph substrate; proposals MUST be treated as non-authoritative unless and until they are captured as commitment objects via the appropriate evidence/authority membrane.

### 2.9 Verification, Evaluation, Validation, Assessment

SOLVER-Ralph distinguishes:

- **Verification (agentic / oracle-driven):** evidence production about conformance to explicit requirements. Output is evidence (not authority).
- **Evaluation (human):** human interpretation of verification evidence. Evaluation is non-binding unless explicitly elevated into a Portal Approval or Decision.
- **Validation (agentic / evidence-backed):** evidence production about fitness in context. Output is evidence (not authority).
- **Assessment (human):** human interpretation of validation evidence. Assessment is non-binding unless explicitly elevated into a Portal Approval or Decision.

**Important:** Evaluation/Assessment are not substitutes for Portal Approval. They may inform work and decisions, but they do not create Verified/Approved/Shippable states.

### 2.10 Paradigm vs Instance

- **Paradigm:** the archetypal set of invariants and boundary constraints that must hold across implementations.
- **Instance:** a concrete deployment/repository that *selects* specific governed artifact versions and configures runtime enforcement.

This SR-CONTRACT defines paradigm-level binding invariants. Instance selection is performed by the governed set (e.g., SR-README registry) and change routing (SR-CHANGE); detailed mechanics live in SR-SPEC and execution sequencing in SR-DIRECTIVE.
...
**Requirement:** Human **Evaluation** and **Assessment** records (non-binding interpretations of evidence) MUST NOT be treated as substitutes for Portal **Approval** or binding **Decision** records. An implementation MUST ensure that:

- only Portal Approvals/Decisions create or change binding authority state, and
- evaluation/assessment artifacts, if recorded, are unambiguously non-binding and cannot advance a Candidate to Approved/Shippable.

**Verified by:** Oracle (state-transition constraints; record typing checks where applicable) + Portal (audit review).
...
### C-EVID-6: Evidence Availability and Retrievability

**Requirement:** Evidence referenced by binding claims MUST remain retrievable.

At minimum:

- Evidence Bundles and any content-addressed logs/outputs they reference MUST be retrievable by their declared digest for as long as they are referenced by any binding claim (Verified, Approval, Freeze, Shippable), **or** a governed retraction/correction MUST be recorded that invalidates/supersedes the claim.
- If referenced evidence cannot be retrieved, the system MUST record an integrity condition **EVIDENCE_MISSING**, MUST treat affected Verified/Approved/Shippable claims as invalid for progression, and MUST route escalation to the appropriate Portal.

**Verified by:** Oracle (retrievability checks; reference-walk tests) + Portal (storage policy audit; incident/exception review).
...
- apply only to explicit FAIL outcomes and MUST NOT waive integrity conditions (see §2.5),
...
The table below is intended to be **mechanically actionable**: it lists every contract requirement, how it is verified (Oracle vs Portal), the minimum evidence artifacts expected to exist, and where the detailed mechanics should live (SR-SPEC vs SR-DIRECTIVE).

| Requirement (C-…) | Verified by | Required evidence artifacts (minimum) | Mechanics owner |
|---|---|---|---|
| **C-ARCH-1** — Hexagonal Separation | Oracle + Portal | Evidence Bundle: architecture conformance; Portal approval: architecture review | SR-SPEC |
| **C-ARCH-2** — Domain Purity | Oracle + Portal | Evidence Bundle: domain/infra separation checks; Portal approval: architecture review | SR-SPEC |
| **C-ARCH-3** — Event Store as Source of Truth | Oracle + Portal | Evidence Bundle: event-log replay/projection tests; Portal approval: architecture review | SR-SPEC |
| **C-TB-1** — Human-Only Binding Authority | Oracle + Portal | Event log + Approval/Decision records show binding actions are HUMAN-only | SR-SPEC |
| **C-TB-2** — Non-Authoritative Agent Output | Oracle + Portal | Oracle checks: agents cannot create Verified/Approved/Shippable state; audit logs | SR-SPEC |
| **C-TB-3** — Portal Crossings Produce Approvals | Oracle + Portal | Approval record(s) for every portal crossing; links to subject + evidence | SR-SPEC |
| **C-TB-4** — Minimum Required Portals | Oracle + Portal | Directive portal registry/config shows required minima; oracle presence checks | SR-DIRECTIVE + SR-SPEC |
| **C-TB-5** — Stable Actor Identity | Oracle + Portal | Auth/audit artifacts; events include stable actor identity | SR-SPEC |
| **C-TB-6** — Approval Record Minimum Fields | Oracle + Portal | Approval record schema-valid w/ required fields; oracle validation output | SR-SPEC |
| **C-TB-7** — Evaluation and Assessment Are Not Approval | Oracle + Portal | Evidence that evaluation/assessment records (if present) are non-binding and cannot advance binding state; portal audit artifacts | SR-SPEC |
| **C-VER-1** — Verification Is Evidence-Based and Candidate-Bound | Oracle | Candidate identity + Evidence Bundle (`evidence.gate_packet`) + (if any) integrity-condition records | SR-SPEC |
| **C-VER-2** — Verified (Strict) | Oracle | Oracle outputs/tests show Strict = all required PASS | SR-SPEC |
| **C-VER-3** — Verified-with-Exceptions | Oracle + Portal | Evidence Bundle includes FAIL; Gate Waiver record(s) + human approval | SR-SPEC |
| **C-VER-4** — Verified Claims Must Declare Mode and Basis | Oracle | Verified claim includes mode + suite pin + waiver refs; oracle metadata validation | SR-SPEC |
| **C-SHIP-1** — Shippable Requires Freeze + Approval + Verified | Oracle + Portal | Freeze record (`record.freeze`) + Release approval + Verified claim + exception refs | SR-SPEC + SR-DIRECTIVE |
| **C-OR-1** — Required Oracles Must Be Deterministic; Advisory Oracles Are Allowed | Oracle + Portal | Determinism/flake checks where possible; governance classification record (required vs advisory) | SR-SPEC + SR-DIRECTIVE |
| **C-OR-2** — Suite Pinning and Integrity | Oracle + Portal | Run pins suite hash; ORACLE_TAMPER record/event on mismatch; (if rebase) decision/approval | SR-SPEC |
| **C-OR-3** — Environment Constraints Must Be Declared and Enforced | Oracle + Portal | Run env fingerprint; ORACLE_ENV_MISMATCH record/event on violation | SR-SPEC |
| **C-OR-4** — Oracle Gaps Are Blocking | Oracle | Completeness check; ORACLE_GAP record/event if missing required oracle result | SR-SPEC |
| **C-OR-5** — Oracle Flake Is a Stop-the-Line Integrity Condition | Oracle + Portal | Repeated-run evidence; ORACLE_FLAKE record/event; governance record if reclassified | SR-SPEC + SR-DIRECTIVE |
| **C-OR-6** — No Silent Oracle Weakening | Oracle + Portal | Governance change/decision record for any required-oracle weakening; prevention checks | SR-SPEC + SR-DIRECTIVE |
| **C-OR-7** — Oracle Integrity Conditions Halt and Escalate | Oracle + Portal | Integrity-condition record + escalation route + portal resolution record | SR-SPEC + SR-DIRECTIVE |
| **C-EVID-1** — Evidence Bundle Minimum Manifest | Oracle + Portal | Evidence Bundle (`evidence.gate_packet`) includes minimum manifest | SR-SPEC |
| **C-EVID-2** — Evidence Immutability and Content Addressing | Oracle + Portal | Content-hash validation evidence; immutable storage/audit logs | SR-SPEC |
| **C-EVID-3** — Evidence Attribution | Oracle | Evidence manifest includes actor identity/type/timestamp; oracle field validation | SR-SPEC |
| **C-EVID-4** — Evidence Dependency References | Oracle + Portal | Evidence manifest includes required refs; oracle presence checks | SR-SPEC |
| **C-EVID-5** — Evidence Containing Secrets | Portal | Restricted original (H1) + optional redacted (H2) + redaction manifest; secret-handling audit | SR-SPEC |
| **C-EVID-6** — Evidence Availability and Retrievability | Oracle + Portal | Retrieval checks for referenced evidence; incident/exception records if evidence missing; storage/audit artifacts | SR-SPEC |
| **C-EVT-1** — Event Attribution | Oracle | Event records include actor attribution; oracle schema validation | SR-SPEC |
| **C-EVT-2** — Append-Only Event Log | Oracle + Portal | Append-only event store guarantees + audit evidence | SR-SPEC |
| **C-EVT-3** — Explicit Supersession for Corrections | Oracle + Portal | Supersedes/retracts links recorded; oracle linkage tests | SR-SPEC |
| **C-EVT-4** — Sequence-First Ordering (Per Stream) | Oracle | Per-stream monotonic sequence numbers; store contract tests | SR-SPEC |
| **C-EVT-5** — Event Graph References for Impact Analysis | Oracle + Portal | Typed ref graph supports dependency vs provenance; oracle graph tests | SR-SPEC |
| **C-EVT-6** — Staleness Marking and Re-Evaluation Routing | Oracle + Portal | Staleness markers recorded + traversal behavior; oracle traversal tests | SR-SPEC |
| **C-EVT-7** — Projections Must Be Derivable From the Audit Trail | Oracle + Portal | Rebuild-from-events tests; architecture review record | SR-SPEC |
| **C-LOOP-1** — Bounded Iteration With Hard Stop | Oracle + Portal | Budget config (Directive) + event log shows accounting + BUDGET_EXHAUSTED halts | SR-SPEC + SR-DIRECTIVE |
| **C-LOOP-2** — Fresh-Context Iterations With Controlled Memory | Oracle + Portal | Iteration contexts are fresh except prior summary; event log + replay evidence | SR-SPEC |
| **C-CTX-1** — Iteration Context Provenance via IterationStarted Refs | Oracle + Portal | `IterationStarted.refs[]` present; oracle schema validation | SR-SPEC |
| **C-CTX-2** — No Ghost Inputs | Oracle + Portal | Context compiler deterministically derives context from IterationStarted payload+refs; tests | SR-SPEC |
| **C-LOOP-3** — Mandatory Stop-the-Line Triggers | Oracle + Portal | Stop-triggers (Directive) + halt events; REPEATED_FAILURE N≥3 declared; tests | SR-SPEC + SR-DIRECTIVE |
| **C-LOOP-4** — Candidate Production Must Be Traceable | Oracle + Portal | Candidate registered w/ stable identity; graph links loop→candidate→run→approval | SR-SPEC |
| **C-EXC-1** — Exceptions Are Records, Not Drift | Oracle + Portal | Deviation/Deferral/Waiver record exists for sustained mismatch; portal review record | SR-SPEC + SR-DIRECTIVE |
| **C-EXC-2** — Binding Exceptions Are Visible at Baseline and Approval | Oracle + Portal | Freeze record lists active exceptions; approvals acknowledge exceptions | SR-SPEC |
| **C-EXC-3** — Exceptions Do Not Silently Rewrite Governance | Portal | Governance review evidence shows exceptions are scoped permissions, not doc rewrites | SR-DIRECTIVE |
| **C-EXC-4** — Gate Waiver Required Fields | Oracle + Portal |Gate Waiver record has required fields + scope + expiry; waiver approval; linkage validation (FAIL outcomes only; not integrity conditions)| SR-SPEC |
| **C-EXC-5** — Waiver Scope Constraints | Oracle + Portal | Scope validation outputs ensure waiver is scoped/time-boxed; escalated approval where needed | SR-SPEC |
| **C-DEC-1** — Binding Decisions Are Recorded | Portal | Decision record includes question/options/decision/rationale + refs; portal review record | SR-SPEC + SR-DIRECTIVE |
| **C-META-1** — Machine-Readable Metadata | Oracle | YAML frontmatter schema-valid per SR-TYPES; oracle metadata validation | SR-SPEC |
| **C-META-2** — Stable Identity and Lineage | Oracle + Portal | Stable IDs + supersedes/lineage refs; oracle lineage validation | SR-SPEC |
| **C-META-3** — Binding Records Are Distinguishable | Oracle + Portal | Typed records distinguish binding vs non-binding; oracle type-key validation | SR-SPEC |
