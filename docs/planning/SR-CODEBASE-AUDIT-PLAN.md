...
- [ ] **P1-BUDGET-GOV** — Align governor budget defaults to SR-DIRECTIVE (5 iterations / 25 oracle runs / 16 hours) and parse `budgets` payload (plural); track `max_oracle_runs` exhaustion in stop logic; tests cover: default budgets, honoring requested budgets, max_oracle_runs exhaustion triggering stop, and monotonic budget patching. (C-LOOP-1, SR-DIRECTIVE §4.1) *(Updated: defaults aligned, `budgets` parsing hardened, oracle-run budget tracked and enforced via RunStarted StopTriggered; budget patch monotonicity helper + tests added; loop creation honors provided budgets; governor applies payload budgets; max_oracle_runs stop now emits HumanAuthorityExceptionProcess recommendation with coverage. Remaining: consider projection/API end-to-end budget honor proof if needed.)*
