...
Platform-definition precedence chain (highest → lowest): SR-CONTRACT → SR-SPEC → SR-TYPES → SR-WORK-SURFACE → SR-SEMANTIC-ORACLE-SPEC → SR-EVENT-MANAGER → SR-AGENT-WORKER-CONTRACT → SR-GUIDE → SR-INTENT.


## Terminology normalization

- **Portal** is the canonical term for a human decision trust boundary (aliases: *Authority Port*, *Trust Boundary*).
- The canonical work-unit surface form is `semantic-ralph-loop` (other variants are aliases).
- Canonical evidence artifact name is `domain.evidence_bundle`.
...
- `governance.charter` is the canonical `doc_kind` for SR-CHARTER (registered in SR-TYPES).

When a document references another SR-* document, it should either:
- link using the **folder path** (preferred), or
- `normative_status` is an enum defined in SR-TYPES; CHARTER uses the same vocabulary. Current values include: `record`, `evidence`, `directional`, `normative`, `index`.
