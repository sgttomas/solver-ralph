...
- **Evidence Bundle:** structured evidence artifact produced by a Run. The SR-TYPES platform domain type key is `domain.evidence_bundle`; the `artifact_type` field value in the manifest schema is `evidence.gate_packet`. These are complementary: the type key classifies what the artifact *is* in the type registry; the artifact_type identifies the manifest schema version.
...
**Naming normalization:** The canonical surface form is `semantic-ralph-loop` (hyphenated, lowercase). Acceptable aliases: "Semantic Ralph Loop" (prose), "Ralph Loop" (informal when context is clear). See SR-CONTRACT §2.3 and §2.11 for the full terminology mapping.
...
2) **Work Surface components** (implements C-CTX-1 for Semantic Ralph Loops)  
   - **Intake:** `kind=Intake`, `rel=depends_on`  
     - MUST carry `meta.content_hash`  
     - References the structured problem statement / objective for this work unit
   ...
   - **Oracle Profile (when semantic oracles are used):** `kind=OracleSuite`, `rel=depends_on`  
     - MUST carry `meta.content_hash`; MUST incorporate semantic set definitions per §1.2.5
   - NOTE: These three components constitute the Work Surface per SR-CONTRACT §2.3. For non-semantic loops, Intake and ProcedureTemplate MAY be omitted if not applicable.

3) **Governing artifacts in force** (binding semantics)  
...
8) **Human intervention notes (non-binding, semantic input)**
...
9) **Agent definition / worker configuration** (audit-only by default)  
...
10) **Gating policy in force** (audit-only by default)
...
**Semantic Loop stop conditions (SR-DIRECTIVE defined):**

The following stop triggers are defined by SR-DIRECTIVE for Semantic Ralph Loops and stage-gated procedures:

- `WORK_SURFACE_MISSING` — intake, procedure template, or stage context absent for a semantic work unit
- `STAGE_UNKNOWN` — stage_id not defined in the bound procedure template
- `SEMANTIC_PROFILE_MISSING` — required stage profile/suite not declared for the current stage

These are waivable stop conditions (not integrity conditions) but require explicit human decision via portal to proceed.
