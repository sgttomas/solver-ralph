...
- **Portal (Authority Port):** gate requiring human arbitration; every Portal is a trust boundary. The terms "Portal" and "Authority Port" are synonymous.
  - *Implementation Note:* A Portal is an abstract trust-boundary process, not necessarily a software UI. During the build/MVP phase, manual human review (e.g., the project owner reviewing and recording a decision) satisfies the Portal requirement. SR-SPEC may define concrete Portal implementations; absence of a "portal product" does not exempt the invariant—it means the human authority functions as the portal directly.
