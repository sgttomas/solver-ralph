...
**Boundary note:** SR-DIRECTIVE is a run-scoped governed policy input consumed by the Event Manager; it configures eligibility/stop policy but does not redefine platform invariants (which remain defined by SR-CONTRACT/SR-SPEC/SR-TYPES).
