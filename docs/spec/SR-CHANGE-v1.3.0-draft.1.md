---
solver_ralph:
  schema: solver_ralph.artifact-metadata/v1
  id: "SR-CHANGE"
  type: "governance.change_policy"
  ...
  ext:
    change_mgmt:
      # Portal *functions* are semantic trust-boundary roles that MUST exist.
      # Portal *ids* are how an instance chooses to implement those functions.
      portal_functions:
        - "GovernanceChange"
        - "ReleaseApproval"
        - "ExceptionApproval"
      ...
      # An instance MAY implement a portal function as a typed request within another portal_id,
      # provided the approval/decision record is explicit, attributable, and references all required evidence.
      portal_function_aliasing:
        allowed: true
        defaults:
          OracleSuiteChange: ["GovernanceChangePortal"]
          PhaseGate: ["GovernanceChangePortal"]
          BudgetExtension: ["GovernanceChangePortal"]
      governed_set:
        canonical_ids:
          - "SR-TYPES"
          - "SR-README"
          ...
          - "SR-CHANGE"
      classification:
        # Change kinds for governance routing. Instances MAY refine with additional sub-kinds.
        kinds:
          - "G:PATCH"   # clarifications, bug fixes, tighten wording without changing meaning
          - "G:MINOR"   # additive / backward-compatible changes (new portals/functions, new gates, new profiles)
          - "G:MAJOR"   # breaking changes (rename/remove semantics, re-define binding terms, remove required invariants)
---
...
### 0.1 1.3.0-draft.1 (this draft)

- Reframed the required portal list as **portal functions** + **instance portal IDs**, enabling **portal-function aliasing** (e.g., OracleSuiteChange can be implemented inside GovernanceChangePortal) while preserving binding approval semantics.
- Expanded the **canonical governed set** to include **SR-PARADIGM** and **SR-ETT** (prevents silent drift in authoring rules and membrane topology).
- Completed the **non-waivable integrity condition** set to include `ORACLE_FLAKE` and `EVIDENCE_MISSING` in addition to `ORACLE_TAMPER`, `ORACLE_ENV_MISMATCH`, `ORACLE_GAP`.
- Added explicit routing guidance for **phase gates** and **budget extensions** (as portal functions), while allowing aliasing.
- Removed brittle SR-CONTRACT clause-number references; this document now references SR-CONTRACT concepts without relying on specific numbering.
...
**Minimum instantiated portal IDs for an instance:**
- `GovernanceChangePortal`
- `ReleaseApprovalPortal`
- `ExceptionApprovalPortal`
...
|---|---|---|---|
| Change to the meaning of a normative artifact (SR-CONTRACT, SR-SPEC, SR-DIRECTIVE, SR-TYPES, SR-CHANGE) | GovernanceChange | GovernanceChangePortal | Requires evidence + explicit approval |
| Change to a directional artifact that is canonical (SR-INTENT, SR-ETT, SR-PARADIGM, SR-README) | GovernanceChange | GovernanceChangePortal | Canonical status means auditability matters |
...
| Deviation / Deferral / Gate Waiver creation or modification | ExceptionApproval | ExceptionApprovalPortal | Exceptions create binding state without editing canonical docs |
...
If an instance uses an SR-PLAN instance and directive-owned control surfaces, the Freeze record MUST also include:

- the **SR-PLAN instance** that defined deliverables/acceptance criteria for the baseline, and
