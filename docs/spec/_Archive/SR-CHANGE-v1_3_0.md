  ...
  ext:
    change_mgmt:
      portal_ids:
        ...
        - "OracleSuiteChangePortal"
        - "ExceptionApprovalPortal"
        - "PhaseGatePortal"
        - "BudgetExtensionPortal"
      governed_set:
        canonical_ids:
          - "SR-PARADIGM"
          - "SR-TYPES"
...
- **Portal registry expansion:** Adds PhaseGatePortal (phase completion approval) and BudgetExtensionPortal (BUDGET_EXHAUSTED resolution). Renames OracleSuiteApprovalPortal → OracleSuiteChangePortal for consistency with SR-DIRECTIVE.
- **Canonical governed set expansion:** Adds SR-PARADIGM (meta-process), SR-ETT (membrane topology), and SR-PLAN (deliverable inventory) to the canonical governed set. Freeze manifests must now include these artifacts.
- **Integrity condition completeness:** Adds ORACLE_FLAKE and EVIDENCE_MISSING to the non-waivable integrity conditions list (§7.1), aligning with SR-DIRECTIVE and SR-CONTRACT.
- **Portal routing matrix expansion:** Adds routing rules for phase gate approvals, budget extensions, and stop-trigger arbitration (§3.2).
- **Stop-trigger portal mapping:** Clarifies which portal handles each stop-trigger type (§8.4).
...
- Requires approval records to include **evidence_refs** and explicit **exceptions_acknowledged** (via SR-CONTRACT C-TB-4).
...
8) Any change to the default selection/promotion policy for `config.agent_definition` artifacts that affects SYSTEM/worker behavior, or
9) Any change to **SR-ETT membrane definitions** or harness topology (what constraints exist, where they are enforced), or
10) Any change to **SR-PLAN deliverable inventory** (adding, removing, or modifying deliverables or their dependencies).
...
|---|---|---|
| **G** | Governance (canonical docs + their meaning/selection) | Changing SR-CONTRACT/SR-SPEC/SR-DIRECTIVE/SR-TYPES/SR-CHANGE/SR-ETT/SR-PLAN/SR-PARADIGM; changing `is_current` for canonical artifacts |
...
4) **Approve**  
   Cross the required portal(s) per §3, producing approval records that satisfy SR-CONTRACT C-TB-4.
...
### 3.1 Portal set (binding)

SOLVER-Ralph defines (at minimum) these portals (see SR-DIRECTIVE for evidence requirements and approver roles):

- `GovernanceChangePortal` — Approve changes to governed artifacts or binding meanings
- `ReleaseApprovalPortal` — Approve candidate release (Verified → Shippable progression)
- `OracleSuiteChangePortal` — Approve oracle suite modifications, resolve integrity conditions
- `ExceptionApprovalPortal` — Approve deviations, deferrals, and gate waivers
- `PhaseGatePortal` — Approve phase completion and cross-phase progression
- `BudgetExtensionPortal` — Approve budget extensions after BUDGET_EXHAUSTED
...
|---|---|---|
| Change to a normative artifact's meaning | GovernanceChangePortal | includes SR-CONTRACT, SR-SPEC, SR-DIRECTIVE, SR-TYPES, SR-CHANGE, SR-ETT, SR-PLAN, SR-PARADIGM |
| Change to SR-INTENT (directional) or SR-README (index) | GovernanceChangePortal | **if** the artifact is canonical (auditability), even if not normative |
| Change to which version is `is_current=true` for any canonical artifact | GovernanceChangePortal | human-approved selection change (esp. normative) |
| Oracle suite registration / required/advisory change / environment constraint change | OracleSuiteChangePortal | may also require GovernanceChangePortal if it changes a canonical rule |
| Oracle integrity condition resolution (TAMPER/GAP/FLAKE/ENV_MISMATCH) | OracleSuiteChangePortal | must fix underlying issue; cannot waive |
| Deviation / Deferral / Gate Waiver creation or modification | ExceptionApprovalPortal | creates binding state; does not edit canonical docs |
| Candidate release (Shippable declaration) | ReleaseApprovalPortal | Freeze creation depends on this approval (see §5.3) |
| Phase completion approval | PhaseGatePortal | required before next phase can start |
| Budget extension after BUDGET_EXHAUSTED | BudgetExtensionPortal | loop resumes only after approval |
| Stop-trigger arbitration (REPEATED_FAILURE) | PhaseGatePortal | human decision on loop continuation |
| Stop-trigger arbitration (GOVERNANCE_TOUCH) | GovernanceChangePortal | governance-touching work requires explicit authorization |
| Stop-trigger arbitration (SECURITY_CRITICAL) | ReleaseApprovalPortal | security issues block release |

### 3.3 Approval records (binding via SR-CONTRACT)

Approval records MUST satisfy **SR-CONTRACT C-TB-4** (minimum fields). SR-CHANGE adds the following binding clarifications for implementers:
...
| `approved_by` | Yes | stable HUMAN identity (C-TB-1, C-TB-2) |
...
- every artifact in the **canonical governed set** (SR-TYPES §4.1), each as `{artifact_id, version, content_hash}`:
  ...
  - `SR-SPEC`
  - `SR-ETT`
  - `SR-PLAN` (or instance-specific `SR-PLAN-INSTANCE-*`)
  - `SR-DIRECTIVE`
  - `SR-CHANGE`
...
Exception record schema is defined in SR-SPEC and scope constraints in SR-CONTRACT (C-EXC-*). At minimum, every exception record MUST include:
...
Exception scopes MUST be explicit and bounded per SR-CONTRACT C-EXC-*.
...
Oracle integrity conditions MUST NOT be bypassed via Gate Waiver. The following are **non-waivable**:

| Condition | Meaning |
|-----------|---------|
| `ORACLE_TAMPER` | Suite hash mismatch during run |
| `ORACLE_ENV_MISMATCH` | Environment constraint violated |
| `ORACLE_GAP` | Required oracle result missing |
| `ORACLE_FLAKE` | Non-deterministic required oracle result |
| `EVIDENCE_MISSING` | Referenced evidence not retrievable |

Resolution requires restart/re-run in compliance or an explicit, human-approved suite rebase/change via OracleSuiteChangePortal.
...
### 8.4 Stop-trigger portal mapping (binding)

The following table defines which portal handles each stop-trigger type:

| Stop Trigger | Primary Portal | Notes |
|---|---|---|
| `ORACLE_TAMPER` | OracleSuiteChangePortal | Non-waivable; must fix suite |
| `ORACLE_GAP` | OracleSuiteChangePortal | Non-waivable; must run missing oracle |
| `ORACLE_ENV_MISMATCH` | OracleSuiteChangePortal | Non-waivable; must fix environment |
| `ORACLE_FLAKE` | OracleSuiteChangePortal | Non-waivable; must fix non-determinism |
| `EVIDENCE_MISSING` | GovernanceChangePortal | Non-waivable; must fix evidence storage |
| `REPEATED_FAILURE` | PhaseGatePortal | Waivable via Decision; N≥3 |
| `BUDGET_EXHAUSTED` | BudgetExtensionPortal | Waivable via extension approval |
| `GOVERNANCE_TOUCH` | GovernanceChangePortal | Requires explicit authorization |
| `SECURITY_CRITICAL` | ReleaseApprovalPortal | Blocks release until resolved |
| `DEPENDENCY_CYCLE` | GovernanceChangePortal | Non-waivable; must fix plan |
...
## Appendix B: Canonical governed set (reference)

The canonical governed set as of v1.3.0:

| Artifact ID | Type | Normative Status | Purpose |
|-------------|------|------------------|---------|
| SR-PARADIGM | governance.meta | normative | Meta-process and stage definitions |
| SR-TYPES | governance.types | normative | Type definitions and precedence |
| SR-README | governance.readme | directional | Navigation and index |
| SR-INTENT | governance.intent | directional | Rationale and objectives |
| SR-CONTRACT | governance.contract | normative | Binding invariants |
| SR-SPEC | governance.spec | normative | Schemas and state machines |
| SR-ETT | governance.topology | normative | Membrane topology and harnesses |
| SR-PLAN | governance.plan | normative | Deliverable inventory and dependencies |
| SR-DIRECTIVE | governance.directive | normative | Execution policy (gates, profiles, budgets) |
| SR-CHANGE | governance.change_mgmt | normative | Change management (this document) |

All artifacts in this set MUST be included in Freeze Record manifests (§5.2).
