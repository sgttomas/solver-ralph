...
solver_ralph:
  ...
  title: SOLVER-Ralph Technical Specification
  ...
  - SR-SPEC@1.0.0
---
# SOLVER-Ralph Technical Specification v1.3.0-draft.1

**Purpose:** Define the technical implementation requirements for SOLVER-Ralph as a governance-first, event-sourced, evidence-backed, self-hosting agentic development framework.
...
**Conformance navigation (selected contract requirements):**

| Contract requirement | Primary SR-SPEC sections |
|---|---|
| C-TB-7 (Evaluation/Assessment ≠ Approval) | §1.2.2, §2.2.1, §1.11.4, §2.3.10 |
| C-EVID-6 (Evidence retrievability) | §1.9.4, §2.3.3, §3.4, Appendix C, Appendix A (`EvidenceMissingDetected`) |
| Non-waivable integrity conditions | §1.9.1, §1.9.4, §3.4, Appendix C |
| Staleness routing lifecycle | §1.13, §2.3.9, Appendix A (`StalenessResolved`) |
...
### 1.3.0-draft.1 (2026-01-11)

- Aligns SR-SPEC with SR-CONTRACT v1.1.0-draft.1, including explicit coverage for **C-TB-7** (Evaluation/Assessment are non-binding) and **C-EVID-6** (evidence retrievability).
- Adds normative terminology alignment for **Proposal vs Commitment Object** and **Verification/Evaluation/Validation/Assessment**.
- Introduces **`EVIDENCE_MISSING`** as a non-waivable integrity condition; specifies detection, recording, escalation, and progression-blocking semantics.
- Tightens **Gate Waiver** scope: waivers apply only to explicit oracle `FAIL` outcomes, never to integrity conditions (including `EVIDENCE_MISSING`).
- Adds non-binding **human judgment record** mechanics (evaluation/assessment notes) with API + projection constraints to prevent authority leakage.
- Fixes internal schema inconsistencies: typed ref `rel` tokens (canonicalizes `produces`/`approved_by`), adds `Approval` to `refs[].kind`, and clarifies `global_seq` in the reference event envelope.
- Completes the staleness lifecycle by specifying a `StalenessResolved` event + API endpoint.
- Updates examples and dependency refs to current governed artifacts (notably SR-CONTRACT v1.1.0-draft.1).

### 1.2.0 (2026-01-10)

- Makes iteration start **SYSTEM-only**: `IterationStarted` MUST be emitted with `actor_kind=SYSTEM` and its authoritative inputs MUST be captured as typed references on `IterationStarted.refs[]`.
- Defines the normative **Iteration Context Ref Set** and **ContextCompiler** semantics (no ghost inputs; deterministic context derived only from `IterationStarted` payload + dereferenced `refs[]`).
- Clarifies reference relationship semantics so that `rel=depends_on` participates in staleness propagation (blocking) while `rel=supported_by` is audit-only (non-blocking by default).
- Tightens the **IterationSummary** minimum schema with minimal shape constraints for `actions[]`, `outcomes`, `next_steps[]`, and `open_risks[]`.
- Strengthens **Candidate identity** policy (Git commit + cryptographic hash) and recommends system verification of declared hashes before treating candidate identity as authoritative.
- Adds a normative **Agent Worker integration** pattern (workers consume `IterationStarted`, compile context from refs, produce Candidates, request Runs, and complete iterations) while keeping orchestration outside the hexagonal domain core.

### 1.0.0 (2026-01-09)

- Adds governed artifact registry semantics + event (`GovernedArtifactVersionRecorded`) for C-META-1..C-META-3.
- Adds Decision record model + event (`DecisionRecorded`) for C-DEC-1.
- Adds Freeze Record model + event (`FreezeRecordCreated`) and Shippable computation rules for C-SHIP-1.
- Adds staleness marking + re-evaluation routing (`NodeMarkedStale`, `ReEvaluationTriggered`) for C-EVT-6.
- Extends canonical ULID identifiers with `freeze_<ULID>` and `stale_<ULID>`.
- Extends API surface for governed artifact registration, decisions, freeze records, shippable status, and staleness workflows.

### 1.0.0-draft.1 (2026-01-09)

- Initial technical specification for the **PostgreSQL + outbox** event store, **PostgreSQL graph projection**, **MinIO evidence storage**, **Podman+gVisor oracle runtime**, **NATS orchestration**, and **Zitadel identity** stack.
- Defines the canonical event envelope, evidence bundle manifest (v1), dependency graph model (v1), API surface (v1), and loop/candidate state machines.
...
**Constraint:** binding claims (Verified, Approved, Shippable) MUST be derivable only from commitment objects referenced in the event/graph substrate. Agent-generated content remains a proposal until it is captured as a commitment object via the appropriate evidence and/or authority membranes.
...
|------|-------|--------|-------------------|
| **Verification** | Agentic (oracle) | Evidence about conformance to explicit requirements | No |
...
**Constraint:** Evaluation and Assessment records MUST NOT be treated as substitutes for Portal Approval, and MUST NOT advance Candidates toward Approved/Shippable.
...
The initial bootstrap implementation SHOULD use synchronous projections for `proj.*` tables and asynchronous projection for `graph.*` if needed.
...
```jsonc
{
  ...
  "dependency_refs": {
    "governed_artifacts": [
      {"id": "SR-CONTRACT", "version": "1.1.0-draft.1", "content_hash": "sha256:..."}
    ],
    "active_exceptions": [
...
- The event MUST include `refs[]` entries of `kind=GovernedArtifact` for:
  - this version (`rel=about`) and
  - any superseded version (`rel=supersedes`), when applicable.
...
```jsonc
{
  ...
  "artifact_manifest": [
    { "artifact_id": "SR-CONTRACT", "version": "1.1.0-draft.1", "content_hash": "sha256:<hex>" },
...
```jsonc
{
  ...
  "root": { "kind": "GovernedArtifact", "id": "SR-CONTRACT@1.1.0-draft.1" },
  "reason_code": "GOVERNED_ARTIFACT_CHANGED",
  "reason_detail": "SR-CONTRACT version changed from 1.0.0-draft.5 to 1.1.0-draft.1",
  "dependents": [
...
```jsonc
{
  ...
  "reason": "Candidate depends on updated governed artifact SR-CONTRACT@1.1.0-draft.1"
}
```
