/**
 * Context Bundle Detail Page
 *
 * Displays a single context bundle with its refs and metadata.
 * Per SR-SPEC, context bundles capture the complete effective context
...
export function ContextBundleDetail(): JSX.Element {
  const { bundleId } = useParams<{ bundleId: string }>();
  const auth = useAuth();
  const [bundle, setBundle] = useState<ContextBundleDetail | null>(null);
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <div className={styles.headerStart}>
          <h1 className={styles.title}>Context Bundle</h1>
      ...
      <Card title="Linked Entities" className={styles.cardSpacing}>
        ...
        {bundle.intake_ref && (
          <div className={styles.infoRow}>
            ...
            <Link to={`/context/intakes/${bundle.intake_ref}`} className={styles.link}>
              {bundle.intake_ref}
            </Link>
          </div>
        )}
...
export default ContextBundleDetail;
