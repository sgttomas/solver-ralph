/**
 * Reference Document Detail Page
 *
 * Displays a single reference document with its metadata and references.
 * Per SR-SPEC, reference documents are content-addressed and can be
...
interface ReferenceDocumentDetailData {
  id: string;
  filename: string;
  media_type: string;
  content_hash: string;
...
export function ReferenceDocumentDetail(): JSX.Element {
  const { documentId } = useParams<{ documentId: string }>();
  const auth = useAuth();
  const [document, setDocument] = useState<ReferenceDocumentDetailData | null>(null);
